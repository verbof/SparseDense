...
extern "C" {
    ...
    void pzdotc_ ( int *n, complex< double > *dot, complex< double > *x, int *ix, int *jx, int *descx, int *incx, complex< double > *y, int *iy,
...
complex< double > c_negone (-1.0,0.0), c_one (1.0,0.0), c_zero (0.0,0.0);
...
//Until now the code can only work with square process grids
	...
	complex_bool=atoi(argv[6]);
        ...
        */
	    ...
	    cout << "Before allocation of B's" << endl;
		...
		cout << "Before sending of blocks" << endl;
                if ( iam!=0 ) {
                    ...
                    MPI_Send ( & ( Dmat.pRows[0] ),Dmat.nrows + 1, MPI_INT,0,iam+size,MPI_COMM_WORLD );
                    MPI_Send ( & ( Dmat.pCols[0] ),Dmat.nonzeros, MPI_INT,0,iam+2*size,MPI_COMM_WORLD );
                    MPI_Send ( & ( Dmat.pData[0] ),Dmat.nonzeros, MPI_CXX_DOUBLE_COMPLEX,0,iam+3*size,MPI_COMM_WORLD );
                    Dmat.clear();
                    if ( *position==0 ) {
                        MPI_Send ( & ( Btsparse.nonzeros ),1, MPI_INT,0,iam+4*size,MPI_COMM_WORLD );
                        MPI_Send ( & ( Btsparse.pRows[0] ),Btsparse.nrows + 1, MPI_INT,0,iam+5*size,MPI_COMM_WORLD );
                        MPI_Send ( & ( Btsparse.pCols[0] ),Btsparse.nonzeros, MPI_INT,0,iam+6*size,MPI_COMM_WORLD );
                        MPI_Send ( & ( Btsparse.pData[0] ),Btsparse.nonzeros, MPI_CXX_DOUBLE_COMPLEX,0,iam+7*size,MPI_COMM_WORLD );
                        Btsparse.clear();
                    }
                } else {
                    ...
                    for ( i=1; i<size; ++i ) {
                        ...
                        if ( nonzeroes>0 ) {
                            ...
                            /*MPI_Get_count(&status, MPI_INT, &count);
                            printf("Process 0 received %d elements of process %d\n",count,i);*/
                            MPI_Recv ( & ( Dblock.pData[0] ),nonzeroes, MPI_CXX_DOUBLE_COMPLEX,i,i+3*size,MPI_COMM_WORLD,&status );
                            /*MPI_Get_count(&status, MPI_DOUBLE, &count);
                            printf("Process 0 received %d elements of process %d\n",count,i);*/
                            Dmat.addBCSRComplex ( Dblock );
                            Dblock.clear();
                        ...
                        if ( i / *dims == 0 ) {
                            ...
                            if ( nonzeroes>0 ) {
                                ...
                                /*MPI_Get_count(&status, MPI_INT, &count);
                                printf("Process 0 received %d elements of process %d\n",count,i);*/
                                MPI_Recv ( & ( Bblock.pData[0] ),nonzeroes, MPI_CXX_DOUBLE_COMPLEX,i,i+7*size,MPI_COMM_WORLD,&status );
                                /*MPI_Get_count(&status, MPI_DOUBLE, &count);
                                printf("Process 0 received %d elements of process %d\n",count,i);*/
                                Btsparse.addBCSRComplex ( Bblock );
                                Bblock.clear();
                    ...
                    //Dmat.writeToFile("Dsparse_red.csr");
                    Btsparse.changeCols(Ddim);
                    create2x2SymBlockMatrixComplex ( Asparse,Btsparse, Dmat, Csparse );
                    Btsparse.clear();
                    Dmat.clear();
	    ...
	    cout << "allocation for AB_sol done" << endl;
            ...
            */
            ...
            make_Sij_denseB_complex ( Asparse, BT_i, B_j, s_BT_i, s_B_j, D, lld_D, AB_sol );
            ...
            */
            ...
            //The Schur complement is factorised (by ScaLAPACK)
            pzpotrf_ ( "U",&Ddim,D,&i_one,&i_one,DESCD,&info );
            if ( info != 0 ) {
                printf ( "Cholesky decomposition of D was unsuccessful, error returned: %d\n",info );
                return -1;
            }
            ...
            pzpotri_ ( "U",&Ddim,D,&i_one,&i_one,DESCD,&info );
            if ( info != 0 ) {
                printf ( "Inverse of D was unsuccessful, error returned: %d\n",info );
                return -1;
            }
            ...
            InvD_T_Block = ( complex< double >* ) calloc ( Dblocks * blocksize + Adim ,sizeof ( complex< double > ) );
            ...
            if ( *position == pcol ) {
                ...
                for ( i=0,j=0; i<Dblocks; ++i,++j ) {
                    if ( j==*dims )
                        j=0;
                    if ( *position==j ) {
                        zgesd2d_ ( &ICTXT2D,&blocksize,&i_one,InvD_T_Block + Adim + i * blocksize,&blocksize,&i_zero,&i_zero );
                    }
                    if ( *position==0 ) {
                        zgerv2d_ ( &ICTXT2D,&blocksize,&i_one,InvD_T_Block + Adim + blocksize*i,&blocksize,&j,&j );
                    }
                }
            }
            ...
            // To minimize memory usage, and because only the diagonal elements of the inverse are needed, X' * S is calculated row by row
            // the diagonal element is calculated as the dot product of this row and the corresponding column of X. (X is solution of AX=B)
            XSrow= ( complex< double >* ) calloc ( Dcols * blocksize,sizeof ( complex< double > ) );
            DESCXSROW= ( int* ) malloc ( DLEN_ * sizeof ( int ) );
            if ( DESCXSROW==NULL ) {
                printf ( "unable to allocate memory for descriptor for AB_sol\n" );
                return -1;
            ...
            //Calculating diagonal elements 1 by 1 of the (0,0)-block of C^-1.
            for ( i=1; i<=Adim; ++i ) {
                pzsymm_ ( "R","U",&i_one,&Ddim,&c_one,D,&i_one,&i_one,DESCD,AB_sol,&i,&i_one,DESCAB_sol,&c_zero,XSrow,&i_one,&i_one,DESCXSROW );
                pzdotc_ ( &Ddim,InvD_T_Block+i-1,AB_sol,&i,&i_one,DESCAB_sol,&Adim,XSrow,&i_one,&i_one,DESCXSROW,&i_one );
            }
            ...
            if ( iam ==0 ) {
                ...
                */
                ...
                printdense_complex ( Adim+Ddim, 1, InvD_T_Block, diagOutFile );
